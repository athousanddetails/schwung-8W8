#!/usr/bin/env bash
# Build the 8W8 module for Ableton Move (aarch64), or natively with NATIVE=1.
#
...
IMAGE_NAME="schwung-8w8-builder"
TARGET="${1:-all}"

if [ ! -f /.dockerenv ]; then
    echo "=== 8W8 build (Docker, ubuntu:22.04 / glibc 2.35) ==="
...
if [ "${NATIVE:-0}" != "1" ] && [ -f "$BUILD_DIR/dsp.so" ]; then
    rm -rf dist/8w8
    mkdir -p dist/8w8
    cp "$BUILD_DIR/dsp.so"  dist/8w8/
    cp src/module.json      dist/8w8/
...
echo; echo "=== Artifacts ==="
find "$BUILD_DIR" -maxdepth 1 -type f \( -name "*.so" -o -name "sc808_*" \) \
     -exec sh -c 'printf "%s\n  " "$1"; file -b "$1"' _ {} \;
