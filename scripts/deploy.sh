#!/usr/bin/env bash
#
# Install the 8W8 module on the Move SAFELY.
#
...
DEST="/data/UserData/schwung/modules/sound_generators/8w8"
BUILD="$SRC/build/dsp.so"

[ -f "$BUILD" ] || { echo "no build/dsp.so — run ./scripts/build.sh sc808 first" >&2; exit 1; }
...
if [ "${RELOAD:-1}" = "1" ]; then
    ...
    python3 "$SRC/scripts/reload_slot.py" "${RHOST:-$HOST}" "${SLOT:-0}" 8w8 \
        || echo "    (could not reload automatically - re-pick 8W8 in the slot)"
fi
